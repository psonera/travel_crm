<x-app-layout>
    @section('title', 'Create Lead Pipeline Stage')
        ...
        <div class="flex-none max-w-full ml-auto mr-auto px-3 w-full">
            <div
                ...
                <div
                    ...
                    <h2 class="text-3xl font-bold">Create Lead Pipeline Stage</h2>

                </div>
                <div class="flex-auto p-6" role="tabpanel">
                    <form role="form" method="POST" action="{{ route('settings.lead_pipeline_stages.store') }}">
                        @csrf
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            <x-inputs.text name="code" label="{{ __('Code') }}" value="{{ old('code') }}"
                                autocomplete="name" autofocus />
                        </div>

                        <div class="mb-4">
                            <x-inputs.text name="probability" label="{{ __('Probability') }}"
                                value="{{ old('probability') }}" required autocomplete="name" autofocus />
                        </div>

                        <div class="mb-4">
                            <x-inputs.text name="sort_order" label="{{ __('Sort Order') }}"
                                value="{{ old('sort_order') }}" required autocomplete="name" autofocus />
                        </div>
                        ...
                        <div class="text-center">
                            <button type="submit"
                                ...
                                Add Lead Pipeline Stage</button>
                        </div>
                    </form>
                </div>
            </div>
