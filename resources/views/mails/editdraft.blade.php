        ...
        <div class="flex-none max-w-full ml-auto mr-auto px-3 w-full">
            <div
                ...
                <div class="flex-auto p-6" role="tabpanel">
                    {{-- @dd($mail); --}}
                    <form role="form" method="POST" action="{{ route('mails.updatedraft', $mail) }}"
                        enctype="multipart/form-data">
                        @csrf

                        <mailto to="{{ $mail->to }}" cc="{{ $mail->cc }}" bcc="{{ $mail->bcc }}"></mailto>
                        @error('cc')
                        ...
                        <div class="mb-4">
                            <x-inputs.text name="subject" id="subject" label="{{ __('Subject') }}"
                                value="{{ old('subject', $mail->subject) }}" required autocomplete="subject" autofocus />
                        </div>
                        <div class="mb-4">
                            <x-inputs.textarea name="content" id='content' label="{{ __('Content') }}"
                                value="{{ old('content', $mail->content) }}" required autocomplete="content" autofocus>
                            </x-inputs.textarea>
                        </div>
                        <div class="mb-4">
                            <label class="p-2 font-semibold text-gray-700">Attachments</label>
                            <attachment data="{{ $mail->getMedia('attachment') }}"></attachment>
                        </div>
                        @error('attachment')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                        <div class="text-center">
                            <button type="submit" name="save"
                                class="inline-block mr-2 px-6 py-3 mt-6 mb-2 font-bold text-center text-white uppercase align-middle transition-all bg-black border-0 rounded-lg cursor-pointer active:opacity-85 hover:scale-102 hover:shadow-soft-xs leading-pro text-size-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 bg-gradient-dark-gray hover:border-slate-700 hover:bg-slate-700 hover:text-white fa fa-paper-plane-o">
                                Send
                            </button>
